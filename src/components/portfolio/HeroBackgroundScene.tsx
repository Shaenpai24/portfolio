...
const FieldMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      vec3 base = vec3(0.02, 0.05, 0.09);
      vec3 cyan = vec3(0.49, 0.83, 1.0) * (flowA * 0.55 + flowB * 0.35);
      vec3 mint = vec3(0.45, 0.94, 0.80) * (node * 0.75 + glow * 0.35);

      vec3 color = base + cyan + mint;
      float vignette = smoothstep(0.92, 0.2, length(centered));
